...
class Ntlm(object):
    """
    Initialises the NTLM context to use when sending and receiving messages to and from the server.
    Supports NTLMv2 authenticate.
    It also brings in the ability to use signing and sealing with session_security and generate a MIC structure.

    @param ntlm_compatibility:  The Lan Manager Compatibility Level to use with the auth message - Default 3
    ...
    Attributes:
        negotiate_flags:    A NEGOTIATE structure that contains a set of bit flags.
                            These flags are the options the client supports and are sent in the negotiate_message
        ...
        session_security:   A SessionSecurity structure that can be used to sign and seal messages sent
    ...
    def create_authenticate_message(self, user_name, password=None, nthash=None, lmhash=None, domain_name=None,
                                    workstation=None, server_certificate_hash=None):
        """
        ...
        @param workstation: The workstation we are using to authenticate with, default is None
        @param server_certificate_hash: The SHA256 hash string of the server certificate (DER encoded)
                                        NTLM is authenticating to. Used for Channel Binding Tokens. If nothing
                                        is supplied then the CBT hash will not be sent.
                                        See messages.py AuthenticateMessage for more details.
        ...
        if self.negotiate_flags & NegotiateFlags.NTLMSSP_NEGOTIATE_SEAL \
                or self.negotiate_flags & NegotiateFlags.NTLMSSP_NEGOTIATE_SIGN:
