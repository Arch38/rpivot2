...
class NegotiateMessage(object):
    ...
    @param negotiate_flags: A NEGOTIATE structure that contains a set of bit flags represenging the client's options
    ...
    Attributes:
        ...
        negotiate_flags: A NEGOTIATE structure that contains a set of bit flags represenging the client's options
        version: Client's Windows version. For debugging only. Set when NTLMSSP_NEGOTIATE_VERSION flag is set.
...
class ChallengeMessage(object):
    ...
    The CHALLENGE_MESSAGE is used by the server to challenge the client to prove its identity, For connection-oriented
    requests, the CHALLENGE_MESSAGE generated by the server is in response to the NEGOTIATE_MESSAGE from the client.
...
class AuthenticateMessage(object):
    ...
    @param server_certificate_hash: The SHA256 hash string of the server certificate (DER encoded) NTLM is authenticating to. This is used to add
                                    to the gss_channel_bindings_struct for Channel Binding Tokens support.
                                    If none is passed through then ntlm-auth will not use Channel Binding Tokens when
                                    authenticating with the server which could cause issues if it is set to only
                                    authenticate when these are present. This is only used for NTLMv2 authentication.
    ...
    def __init__(self, user_name, password, nthash, lmhash, domain_name, workstation, challenge_message,
                 ntlm_compatibility, server_certificate_hash):
        self.signature = NTLM_SIGNATURE
